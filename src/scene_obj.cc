...
struct obj_file {
	std::string cur_obj, cur_mat;
	std::vector<Vector3> v, vn, vt;
...
static bool read_materials(FILE *fp, std::vector<obj_mat> *vmtl);
...
static std::map<std::string, int> matnames;
